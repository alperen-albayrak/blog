...
Sürekli rastgele sayılar elde etmek için perlin noise adındaki bir yöntemi kullanacağız. Bu yöntemde farklı frekans ve genlikte sinüs dalgaları bir araya getirilerek sürekli rastgele sayılar oluşuturulur.
...
```javascript showLineNumbers copy {15}
xoff = 0;
start = 0;
inc = 0.01;
function setup() {
...

{
    ...
    >
        ...
        >
            x'e bağlı süreksiz rastgele sayı
        </div>
    </div>
    </>
}
...

{
    ...
    >
        ...
        >
            x'e bağlı sürekli rastgele sayı
        </div>
    </div>
    </>
}
...
Piksellerin kırmızı, yeşil ve mavi renk değerlerine farklı değerler atarsak nasıl bir görsel ortaya çıkacağınıı da aşağıdaki kodla deneyebiliriz. Farklı perlin noiseler elde etmek için kırmızı değerini olduğu gibi bırakırken yeşilin değerlerine +1000 mavinin değerlerine +2000 ekledim. Bu şekilde her değerin farklı noktalardan örnek almasını sağladım.
...
function draw() {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      // Generate noise values for RGB channels
      let r1 = noise(xoff, yoff, zoff) * 255;
      let r2 = noise(xoff + 1000, yoff + 1000, zoff + 1000) * 255;
      let r3 = noise(xoff + 2000, yoff + 2000, zoff + 2000) * 255;
...
- [`Süreksiz Rastgele Sayı Grafiği`](https://github.com/alperen-albayrak/depo/blob/main/p5js/random2d.html)
- [`Sürekli Rastgele Sayı Grafiği`](https://github.com/alperen-albayrak/depo/blob/main/p5js/noise2d.html)
